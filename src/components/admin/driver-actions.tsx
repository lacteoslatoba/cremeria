...
export function DriverActions({ driver }: { driver: Driver }) {
    ...
    const handleDelete = async () => {
        if (!confirm(`¿Estás seguro de que deseas eliminar al repartidor ${driver.name || "sin nombre"}? Sus entregas ya hechas se siguen mostrando en el historial de ventas.`)) return;
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/users/${driver.id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    name, username, phone, role: "DELIVERY",
                    ...(password ? { password } : {}),
                }),
            });
            ...
            if (res.ok) {
                ...
                setError(data.error || "Error al actualizar el repartidor.");
            }
        } catch (error) {
            console.error(error);
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 text-left">
                        <div className="flex justify-between items-center p-6 border-b border-gray-100">
                            <h3 className="text-xl font-bold text-gray-900">Editar Repartidor</h3>
                        ...
                        <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-2">Nueva contraseña (opcional)</label>
                                <input
