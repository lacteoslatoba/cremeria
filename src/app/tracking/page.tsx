...
function TrackingContent() {
    ...
    if (loading) {
        return (
            <>
                {paidBanner}
                <div className="absolute inset-0 flex justify-center items-center bg-white z-20"><Loader2 className="animate-spin text-primary" size={40} /></div>
            </>
        );
    }
    ...
    if (!orderId || !status) {
        return (
            <>
                {paidBanner}
                <div className="absolute inset-0 flex flex-col gap-3 justify-center items-center bg-white z-20 text-gray-900">
    ...
    // Repartidor en camino con GPS real -- estilo DiDi Food: mapa real en
    // vez de la foto/línea de placeholder, y la etiqueta pasa de "Llegada
    // Estimada" a "Distancia". Toda la pantalla usa el mismo tema claro
    // sin importar isLive; esto solo decide qué CONTENIDO mostrar.
    ...
    return (
        <>
            {/* Fondo de toda la pantalla -- cubre el hueco entre el mapa y la
                tarjeta inferior. Toda la pantalla usa el mismo tema claro,
                haya o no ubicación GPS real todavía. */}
            ...
            {/* Map Area -- "isolate" es clave: Leaflet le pone z-index 200-700
                ...
                donde se superponen. */}
            <div className="absolute inset-0 top-0 h-[55%] w-full flex items-center justify-center isolate bg-white">
                {isLive ? (
                    <LiveMap
                ...
                {!isLive && (
                    <div className="relative z-20 w-full h-full flex flex-col justify-center items-center">
                        <div className={`h-32 w-1 border-l-2 border-dashed relative transition-all duration-1000 ${step2Active && !step3Active ? "border-primary drop-shadow-[0_0_8px_rgba(238,43,52,0.8)] animate-pulse" : "border-gray-300"}`}>
            ...
            {/* Bottom Sheet Card -- max-h + overflow-y-auto: en pantallas bajas
                ...
                Toda la tarjeta usa el mismo tema claro sin importar si ya
                hay GPS real o no. */}
            <div className="absolute bottom-16 left-0 right-0 max-w-[480px] mx-auto z-30 pt-6 px-6 pb-8 border-t rounded-t-3xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto bg-white border-black/5 shadow-[0_-10px_40px_rgba(0,0,0,0.15)]">
                <div className="w-12 h-1.5 rounded-full mx-auto mb-6 bg-black/10" />

                {isCancelled ? (
                    <div className="text-center mb-8">
                        <p className="text-red-500 text-sm font-bold uppercase tracking-wider mb-2">Pedido Cancelado</p>
                        <h2 className="text-2xl font-black text-gray-900">Contacta soporte</h2>
                    </div>
                ) : (
                    <>
                        <div className="text-center mb-8">
                            <p className="text-sm font-medium uppercase tracking-wider mb-2 text-gray-500">
                            ...
                            <p className="text-xs mt-2 text-gray-400">Folio: #{orderId.slice(-6).toUpperCase()}</p>
                        </div>

                        {/* Código de verificación de entrega: el cliente se lo da al repartidor */}
                        {deliveryCode && !step3Active && !isCancelled && (
                            <div className="mb-8 p-4 rounded-2xl border text-center bg-primary/10 border-primary/30">
                                <p className="text-[11px] font-medium uppercase tracking-wider mb-2 text-gray-500">
                                    Tu código de entrega
                                </p>
                                <p className="text-3xl font-black tracking-[0.35em] text-gray-900">
                                    {deliveryCode}
                                </p>
                        ...
                        <div className="flex justify-between items-center mb-10 px-2 transition-all">
                            ...
                            <div className="flex flex-col items-center gap-2">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 ${step1Active ? "bg-primary text-white shadow-[0_0_12px_rgba(238,43,52,0.6)]" : "border-2 border-gray-300 text-gray-400"}`}>
                                    <CheckCircle2 size={18} />
                                </div>
                                <span className={`text-[10px] font-semibold transition-colors ${step1Active ? "text-gray-900" : "text-gray-500"}`}>Preparando</span>
                            </div>

                            <div className={`flex-1 h-0.5 mx-2 transition-all duration-500 ${step2Active ? "bg-primary drop-shadow-[0_0_4px_rgba(238,43,52,0.5)]" : "bg-gray-200"}`} />

                            {/* Step 2 */}
                            <div className="flex flex-col items-center gap-2">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 ${step2Active ? "bg-primary text-white shadow-[0_0_12px_rgba(238,43,52,0.8)] animate-pulse" : "border-2 border-gray-300 text-gray-400"}`}>
                                    <MapPin size={18} />
                                </div>
                                <span className={`text-[10px] font-semibold transition-colors ${step2Active ? "text-gray-900" : "text-gray-500"}`}>En Camino</span>
                            </div>

                            <div className={`flex-1 h-0.5 mx-2 transition-all duration-500 ${step3Active ? "bg-primary drop-shadow-[0_0_4px_rgba(238,43,52,0.5)]" : "bg-gray-200"}`} />

                            {/* Step 3 */}
                            <div className="flex flex-col items-center gap-2">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 ${step3Active ? "bg-green-500 text-white shadow-[0_0_12px_rgba(34,197,94,0.6)]" : "border-2 border-gray-300 text-gray-400"}`}>
                                    <CheckCircle2 size={18} />
                                </div>
                                <span className={`text-[10px] font-semibold transition-colors ${step3Active ? "text-green-500 font-bold" : "text-gray-500"}`}>Entregado</span>
                            </div>
                        </div>
                    </>
                )}
                ...
                {/* Driver Info */}
                <div className={`flex items-center justify-between p-4 border rounded-2xl shadow-inner transition-opacity duration-1000 bg-gray-50 border-gray-200 ${(!step2Active || step3Active || isCancelled) ? "opacity-50 grayscale" : "opacity-100"}`}>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-xs font-medium mb-1 text-gray-500">Tu Repartidor</p>
                            <h4 className="font-bold text-gray-900">{delivery?.name || "Por asignar"}</h4>
                        </div>
                    </div>
                    {delivery?.phone ? (
                        <a
                        ...
                        <button disabled className="flex items-center justify-center w-12 h-12 rounded-full cursor-not-allowed bg-gray-100 text-gray-400 border border-gray-200">
                            <PhoneCall size={20} />
                        </button>
                    )}
                </div>
...
export default function TrackingPage() {
    return (
        <main className="min-h-[100dvh] bg-background text-foreground relative overflow-hidden">
            ...
            <Suspense fallback={<div className="absolute inset-0 flex justify-center items-center bg-white z-20"><Loader2 className="animate-spin text-primary" size={40} /></div>}>
                <TrackingContent />
            </Suspense>
